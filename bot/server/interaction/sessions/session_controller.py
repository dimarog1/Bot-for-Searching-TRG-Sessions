...
class SessionController:
    ...
    async def start_session_creation(
        ...
        self.sessions_data[tg_id].master_id = (
            await UserDao.get_user_by_tg_id(tg_id)
        ).id_
        return ConversationStates.GAME_NAME

    async def input_game_name(
        self, update: Update, context: ContextTypes.DEFAULT_TYPE
    ) -> ConversationStates:
        tg_id = update.message.from_user.id
        ...
        await update.message.reply_text(
            "Планируете провести игру онлайн?", reply_markup=reply_markup
        )
        return ConversationStates.IS_ONLINE

    async def input_is_online(
        self, update: Update, context: ContextTypes.DEFAULT_TYPE
    ) -> ConversationStates:
        ...
        if is_online:
            ...
            return ConversationStates.MAX_PLAYERS
        else:
    ...
    async def input_country(
        ...
        return ConversationStates.CITY
    ...
    async def input_max_players(
        ...
        try:
            ...
            await update.message.reply_text(
                "Пожалуйста, укажите корректное количество игроков (от 2 до 20):"
            )
            return ConversationStates.MAX_PLAYERS
        ...
        await update.message.reply_text(
            f"Выберите дату ({STEP_RU[step]}):", reply_markup=calendar
        )
        return ConversationStates.START_DATE
    ...
    async def input_start_date(
        ...
        if not result and key:
            await query.edit_message_text(
                f"*Дата проведения ({STEP_RU[step]})*",
                reply_markup=key,
                parse_mode="Markdown",
            )
            return ConversationStates.START_DATE
        ...
        await query.edit_message_text(
            f"Сессия состоится в этот день: *{result.strftime('%d.%m.%Y')}*",
            parse_mode="Markdown",
        )
        await query.message.reply_text(
            "Во сколько начнем?", reply_markup=self.get_time_keyboard()
        )
        return ConversationStates.START_TIME
            ...
            buttons.append(
                [
                    InlineKeyboardButton(
                        f"{hour:02d}:00", callback_data=f"{hour:02d}:00"
                    ),
                    InlineKeyboardButton(
                        f"{hour:02d}:30", callback_data=f"{hour:02d}:30"
                    ),
                ]
            )
    ...
    async def input_start_time(
        ...
        self.sessions_data[tg_id].start_datetime = datetime.datetime.combine(
            self.sessions_data[tg_id].start_datetime,
            datetime.datetime.strptime(time_str, "%H:%M").time(),
        )

        await query.edit_message_text(
            f"Игра начнется в *{time_str}* по местному времени.", parse_mode="Markdown"
        )
        await query.message.reply_text(f"На сколько часов она планируется?")

        return ConversationStates.DURATION_HOURS

    async def input_duration_hours(
        self, update: Update, context: ContextTypes.DEFAULT_TYPE
    ) -> ConversationStates:
        try:
            ...
            await update.message.reply_text(
                "Пожалуйста, укажите корректную продолжительность сессии (от 1 до 10 часов):"
            )
            return ConversationStates.DURATION_HOURS
